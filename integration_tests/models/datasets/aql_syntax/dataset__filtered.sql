...
append first ever visited_page (
    ...
    filter {{dbt_activity_schema.json_extract('{feature_json}', 'referrer_url')}} = 'yahoo.com'
)
aggregate all bought_something (
    count(activity_id) as total_large_purchases
    filter cast(nullif({{dbt_activity_schema.json_extract('{feature_json}', 'total_sales')}}, '') as int) > 100
    filter cast(nullif({{dbt_activity_schema.json_extract('{feature_json}', 'total_items_purchased')}}, '') as int) > 3
)
include (
